...
def before_request():
    if session.get("logged_in"):
        if session.get('active_hand'):
            flash("Please finish your hand")
            return redirect('/play'), dict(logged_in=session.get("logged_in"),
                                           show_footer=True)
...
# Start of the game
    ...
    while True:
        ...
        if bet.isdecimal():
            ...
            continue
        money -= bet
        session['active_hand'] = False  # activate the hand
...
def login():
    ...
    if username is not None:
        ...
        if password == results[1]:
            ...
            session['active_hand'] = False  # variable to check if the player is in an active hand
            return redirect("/dashboard")
        else:
            flash("Your Username or Password is incorrect")
            return render_template("login.html",
